...
def clone_all_releases(tags_commits, repo_url ) :
    ...
    # in this file we have all the tag with it's correspondante commit
    ...
    print("=====start cloning releases=======\n")
    for index, line in enumerate(lines) :
           
        tag=line.split(',')[0].split("refs/tags/")[1].replace(" ","")
        commit=line.split(',')[1]
        # check if the commit wasn't treated before 
...
def run_szz_per_release(path_tags_commits,repo_url,issue_path,project_local,pattern) :
    ...
    """ the releases are in the chronological order we inversed 
    to start with the most recent one because the fix commit
    is in it's last appareance in recent release , and we search for commit intro in the releases
    starting from the release in which the issue fix and the previous releases """ 
    
    tags=tags[::-1]
    commits=commits[::-1]
        
    releases_file=[]
    print("=======start extracting fix commits=======\n")
    for index ,tag in enumerate(tags) :
        ...
        ## if isse path end ".csv" the issues are from bugzilla or jira 
        if ".csv" in issue_path:
            issue_list=find_bug_fixes(issue_path,gitlog_path,pattern)
        ## if json the issues are from github     
        else :
        ...
        if issue_list !={} :
            ...
            # keep track of the releases that we have found issue fix in it 
    ...
    print("======extract commits intorducing bugs=======")
    # to exttract commit introducing bugs
